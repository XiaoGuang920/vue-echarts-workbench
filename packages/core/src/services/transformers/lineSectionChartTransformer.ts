...
export class LineSectionChartTransformer implements ChartTransformer {
  ...
  async transform(input: ExtendedEChartsOption): Promise<ExtendedEChartsOption> {
    ...
    const series =
      inputSeries
        .map((s: unknown, index: number) => {
          ...
          if (
            !series.data ||
            (Array.isArray(series.data) && (series.data as unknown[]).length === 0)
          )
            return
          ...
          const markLineObj = (series.markLine || {}) as Record<string, unknown>
          const markAreaObj = (series.markArea || {}) as Record<string, unknown>
          const markAreaItemStyleObj = (markAreaObj.itemStyle || {}) as Record<string, unknown>

          const seriesColor =
            lineStyleObj.color || itemStyleObj.color || chartColors[index % chartColors.length]

          return {
            ...series,
            name: (series.name as string) || `Line ${index + 1}`,
            type: (series.type as string) || 'line',
            data: series.data,
            lineStyle: {
              ...lineStyleObj,
              color: seriesColor,
            ...
            markArea: {
              ...markAreaObj,
              silent: (markAreaObj.silent as boolean) ?? true,
              itemStyle: {
                ...markAreaItemStyleObj,
                color: (markAreaItemStyleObj.color as string) || `${accentColors.accent}30`,
              },
              data: (markAreaObj.data as unknown[]) || [],
            },
          }
        })
    ...
    const chartOptions = {
      ...
      title: {
        ...titleObj,
        text: (titleObj.text as string) || 'Line Sections Chart',
      ...
      grid: {
        ...gridObj,
        left: (gridObj.left as string) || '8%',
        right: (gridObj.right as string) || '10%',
        top: (gridObj.top as string) || '20%',
        bottom: (gridObj.bottom as string) || '15%',
