...
/**
 * Scatter Chart JSON Transformer
 */
export class ScatterChartTransformer implements ChartTransformer {
  /**
   * 轉換圖表配置
   * @param input 輸入的圖表配置
   * @returns 轉換後的圖表配置
  ...
  async transform(input: ExtendedEChartsOption): Promise<ExtendedEChartsOption> {
    ...
    const series = inputSeries
      .map((s: unknown, index: number) => {
        ...
        return {
          ...series,
          name: (series.name as string) || `Scatter ${index + 1}`,
          type: (series.type as string) || 'scatter',
          data: seriesData,
          itemStyle: {
            ...seriesItemStyleObj,
            color: seriesColor,
    ...
    const chartOptions = {
      ...
      gridX: input.gridX || 0,
      gridY: input.gridY || 0,
      tooltipType: input.tooltipType || 'scatterLight',
      series: series,
      title: {
        ...titleObj,
        text: (titleObj.text as string) || 'Scatter Chart',
      ...
      xAxis: {
        ...
        min: xAxisObj.min as number | string | undefined,
        max: xAxisObj.max as number | string | undefined,
        interval: xAxisObj.interval as number | undefined,
      },
      yAxis: {
        ...yAxisObj,
        ...
        min: yAxisObj.min as number | string | undefined,
        max: yAxisObj.max as number | string | undefined,
        interval: yAxisObj.interval as number | undefined,
      },
      legend: {
        ...legendObj,
